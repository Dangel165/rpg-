...
@plugindesc ♦1.3.2♦ Add class, equips, level and exp to Enemies!
@author Hakuen Studio
@url https://hakuenstudio.itch.io/eli-enemy-class-for-rpg-maker/rate?source=game
...
Eli.EnemyClass = {
    ...
    Parameters: class{
        constructor(parameters){
            this.equipRules = parameters.equipRules === "true"
            this.actionPatternMode = parameters.actionPatternMode
            this.defaultActionRating = Number(parameters.defaultActionRating)
        }
    },
    ...
    setCallbackForDataChange(){
        ...
        }[this.getParam().actionPatternMode]

        if(callback){

            this.processDataEnemies = (data) => {
                if(data.meta.hasOwnProperty("ClassId")){
                    callback(data)
                }
            }
        }
    ...
    addActionPatternsToDataEnemy(data){
        ...
        for(const learning of currentClass.learnings){
            ...
            if(action){
                ...
                const actionRating = this.getParam().defaultActionRating
    ...
    needChangeDataEnemies(){
        return  this.getParam().actionPatternMode === "Learn and Require Level" || 
                this.getParam().actionPatternMode === "Require Level"
    },
